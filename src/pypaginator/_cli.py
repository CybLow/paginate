"""CLI entry point for pypaginator development commands.

This module provides development utilities accessible via `uv run pypaginator <cmd>`.
For production use, prefer direct tool invocation or UV scripts.

Usage:
    uv run pypaginator lint          # Check code with ruff
    uv run pypaginator lint --fix    # Auto-fix linting issues
    uv run pypaginator format        # Format code with ruff
    uv run pypaginator typecheck     # Run mypy type checking
    uv run pypaginator test          # Run test suite
    uv run pypaginator test -x       # Run tests, stop on first failure
    uv run pypaginator quality       # Run all quality checks
    uv run pypaginator build         # Build distribution packages
    uv run pypaginator clean         # Remove build artifacts
"""
...
def _show_help() -> NoReturn:
    ...
    help_text = """
{header}pypaginator - Development CLI{reset}

{bold}Usage:{reset}
    uv run pypaginator <command> [options]
...
{bold}Examples:{reset}
    uv run pypaginator lint --fix
    uv run pypaginator test -x -v
