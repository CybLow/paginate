...
```{eval-rst}
.. automodule:: pypaginate.database.types
   :members: SelectStatement, CountStatement
   :undoc-members:
   :show-inheritance:
```
...
Describes the SQL statements needed to set up text collations for search operations.

```{eval-rst}
.. autoclass:: pypaginate.database.CollationPlan
   :members:
   :show-inheritance:
```
...
```{eval-rst}
.. autofunction:: pypaginate.database.ensure_database_collations
```

```{eval-rst}
.. autofunction:: pypaginate.database.recommend_collation_plan
```
