...
Normalizer for in-memory text search operations. Applies case folding, accent removal, and Unicode normalization.

```{eval-rst}
.. autoclass:: pypaginate.text.MemoryTextNormalizer
   :members:
   :show-inheritance:
```
...
Normalizer for SQL-based text search. Prepares text for database LIKE queries.

```{eval-rst}
.. autoclass:: pypaginate.text.SqlTextNormalizer
   :members:
   :show-inheritance:
```
