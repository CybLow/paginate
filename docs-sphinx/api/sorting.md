...
Generic sorting engine for in-memory collections. Supports null handling and tie-breaking.

```{eval-rst}
.. autoclass:: pypaginate.sorting.SortEngine
   :members:
   :show-inheritance:
```
...
SQL adapter for building SQLAlchemy ORDER BY expressions with null positioning.

```{eval-rst}
.. autoclass:: pypaginate.sorting.SqlSortAdapter
   :members:
   :show-inheritance:
```
...
```{eval-rst}
.. autofunction:: pypaginate.sorting.sort_items
```

```{eval-rst}
.. autofunction:: pypaginate.sorting.create_sort_service
```

## Types

```{eval-rst}
.. autoclass:: pypaginate.sorting.Nulls
   :members:
   :show-inheritance:
```
